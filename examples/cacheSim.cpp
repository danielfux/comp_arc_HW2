...
public:
    ...
    void make_block_dirty(unsigned int block_num, unsigned int set_i) { ///
        sets_vec[set_i]->blocks_vec_in_set[block_num]->dirty_ = true;///
        sets_vec[set_i]->lru_update(block_num); ///
    ...
    switch (state) {
        case 0: { // miss in L1 and miss in L2
            ...
            if (L1->sets_vec[l1_set]->isLruDirty()) {
                unsigned int lru_tag = L1->sets_vec[l1_set]->return_lru_tag();
                updateL2IfL1Dirty(L1, L2,l1_tag, l2_set, lru_tag);
                evict_l2(L1,  l1_tag, L2, l2_set, lru_tag);
            }
            L1->sets_vec[l1_set]->addBlockToSet(l1_tag,address);
            L2->sets_vec[l2_set]->addBlockToSet(l2_tag,address);//maintain inclusion principle
            break;
        }
        ...
        case 1: { // miss in L1 and hit in L2
            ...
            if(L1->sets_vec[l1_set]->isLruDirty()) {
                unsigned int lru_idx = L1->sets_vec[l1_set]->return_lru_idx();
                unsigned int lru_address = L1->sets_vec[l1_set]->blocks_vec_in_set[lru_idx]->address_;
                unsigned int parallel_set_to_l1 = L2->idxOfSetInCache(lru_address,L2->num_of_sets_in_a_cache_);
                unsigned int parallel_tag_to_l1 = L2->get_tag(lru_address,L2->bits_for_set_);
                L2->sets_vec[parallel_set_to_l1]->addDirtyBlockToSet(parallel_tag_to_l1,lru_address);
            }
            L1->sets_vec[l1_set]->addBlockToSet(l1_tag,address);
            break;
        }
...
void writeToCache(Cache *L1, unsigned int l1_set, unsigned int l1_tag,
    ...
    switch (state) {
        ...
        case 1: { // miss in L1 and hit in L2
            ...
            L1->times_to_access_cache_++;
            L2->times_to_access_cache_++;
            L2->sets_vec[l2_set]->addDirtyBlockToSet(l2_tag,address);
            if (write_allocate == 1) {
                if (L1->sets_vec[l1_set]->isLruDirty()) {
                    //unsigned  int lru_tag = L1->sets_vec[l1_set]->return_lru_tag();
                    //updateL2IfL1Dirty(L1, L2,l1_tag, l2_set, lru_tag);
...
void getStats(double *L1MissRate, double *L2MissRate, double *avgAccTime, Cache *L1, Cache *L2) {
    *L1MissRate = 0, *L2MissRate = 0, *avgAccTime = 0;
    ...
    if (times_to_access_data != 0)
        *avgAccTime = (double) tot_time_to_access_data / times_to_access_data;
}
